...
#df['date'] = df['text'].apply(lambda x:re.findall(r'\d{1,2}\/\d{1,2}\/\d{2,4}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|[A-Z][a-z]+[/-]\d{1,2}[/-]\d{2,4}|[A-Z][a-z]{2,}[,.]? \d{1,2}[a-z]{2,},? \d{2,4}|\d{1,2} [A-Z][a-z]{2}[,.]+ \d{2,4}|[A-Z][a-z]{2,}[,.]? \d{2,4}|\d{1,2}\/\d{4}|\d{4}',x)) 
df['date'] = df['text'].apply(lambda x:re.findall(r'\d{1,2}\/\d{1,2}\/\d{2,4}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[/-]\d{1,2}[/-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[,.]? \d{1,2}[a-z]{2},? \d{2,4}|\d{1,2} [A-Z][a-z,.]+ \d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[,.]? \d{2,4}|\d{1,2}\/\d{4}|[12]\d{3}',x)) 
...
df['date'][298]=('January 1993')
df['date'][313]=('December 1 1978')
...
for year in L:
    if(re.match(r'\d{4}',year)) :
        ...
        L[i] = date_split[0] + '/1/'+date_split[1]
        year = L[i]
    elif(re.match(r'[A-Z][a-z]+[,.]? \d{4}',year)) :
        date_split = year.split(' ')
        L[i] = date_split[0] + ' 1 '+date_split[1]
        year = L[i]
    L[i] = dateutil.parser.parse(L[i]).strftime("%m/%d/%Y")
    i = i+1
...
df['index'] = sorted(range(len(L)), key=lambda x : fun(L[x]))
